...
### COHO
# dividing into halfs, thirds, and fourths requires twelvths
cutoffs <- quantile(nosa_coho$lnnosa, probs = c(1/12, 1/6, 1/4, 1/3, 5/12, 1/2, 7/12, 2/3, 3/4, 5/6, 11/12), na.rm = TRUE)
                                                                    ...
                                                                    ifelse(nosa_coho$lnnosa <= cutoffs[8], "Q8",
                                                                           ifelse(nosa_coho$lnnosa <= cutoffs[9], "Q9",
                                                                                  ifelse(nosa_coho$lnnosa <= cutoffs[10], "Q10",
                                                                                         ifelse(nosa_coho$lnnosa <= cutoffs[11], "Q11", "Q12"
                                                                                         )))))))))))
...
nosa_coho_rows$method2 <- ifelse(nosa_coho_rows$nth %in% c("Q1", "Q2", "Q3", "Q4", "Q5", "Q6"), "S", "L")
nosa_coho_rows$method2 <- paste0(nosa_coho_rows$methodNo, "",nosa_coho_rows$method2)

nosa_coho_rows$method3 <- ifelse(nosa_coho_rows$nth %in% c("Q1", "Q2", "Q3", "Q4"), "S", 
                                 ifelse(nosa_coho_rows$nth %in% c("Q5", "Q6", "Q7", "Q8"), "M", "L"
                                 ))
nosa_coho_rows$method3 <- paste0(nosa_coho_rows$methodNo, "",nosa_coho_rows$method3)

nosa_coho_rows$method4 <- ifelse(nosa_coho_rows$nth %in% c("Q1", "Q2", "Q3"), "Q1", 
                                 ifelse(nosa_coho_rows$nth %in% c("Q4", "Q5", "Q6"), "Q2",
                                        ifelse(nosa_coho_rows$nth %in% c("Q7", "Q8", "Q9"), "Q3", "Q4"
                                        ))) 
nosa_coho_rows$method4 <- paste0(nosa_coho_rows$methodNo, "",nosa_coho_rows$method4)
...
POPSIZE4R_coho.model <- matrix(list(0), n_coho, n_coho)
diag(POPSIZE4R_coho.model) <- paste0("r", nosa_coho_rows$method4)
...
n_coho <- nrow(nosa_coho)
POPSIZE2R_coho.model <- matrix(list(0), n_coho, n_coho)
diag(POPSIZE2R_coho.model) <- paste0("r", nosa_coho_rows$method2)
...
# sets relative value against which other survey methods will be scaled
  # 9 -> dam counts - accurate (according to parsons and Skalski)
  # 9s is the only 9 method that appears across all three species
POPSIZE4a_coho.model <- matrix(list(0), n_coho, 1)
for(i in 1:length(POPSIZE4a_coho.model)){
  if(nosa_coho_rows$method4[i] != scale4){
    POPSIZE4a_coho.model[i] <- paste0("a", nosa_coho_rows$method4[i])
  }
}
...
for(i in 1:length(POPSIZE2a_coho.model)){
  if(nosa_coho_rows$method2[i] != scale2){
    POPSIZE2a_coho.model[i] <- paste0("a", nosa_coho_rows$method2[i])
  }
}
...
POPSIZE2mod_coho.list <- list(
  B = "identity",
  U = "zero",
  Q = "equalvarcov",
  Z = Z_coho.model,
  A = POPSIZE2a_coho.model,
  R = POPSIZE2R_coho.model,
  x0 = "equal",
  V0 = "zero",
  tinitx = 0
)
...
POPSIZE4mod_coho.list <- list(
  B = "identity",
  U = "zero",
  Q = "equalvarcov",
  Z = Z_coho.model,
  A = POPSIZE4a_coho.model,
  R = POPSIZE4R_coho.model,
  x0 = "equal",
  V0 = "zero",
  tinitx = 0
)
...
# agnostic to pop size
if(!file.exists(here::here("data", "clean", paste("POPSIZE1ssm_cohoM", scale1, "S4TEST.rds", sep="")))){
  ptm <- proc.time()
  POPSIZE1ssm_coho <- MARSS(nosa_coho, model = POPSIZE1mod_coho.list, method = "kem", control = con.list)
  saveRDS(POPSIZE1ssm_coho, file=here::here("data", "clean", paste("POPSIZE1ssm_cohoM", scale1, "S4TEST.rds", sep="")))
  coho_time <- proc.time()[3] - ptm
  coho_time
}
# load in ssm_coho
POPSIZE1ssm_coho <- readRDS(file=here::here("data", "clean", paste("POPSIZE1ssm_cohoM", scale1, "S4TEST.rds", sep="")))

# large and small
if(!file.exists(here::here("data", "clean", paste("POPSIZE2ssm_cohoM", scale2, "S4TEST.rds", sep="")))){
  ptm <- proc.time()
  POPSIZE2ssm_coho <- MARSS(nosa_coho, model = POPSIZE2mod_coho.list, method = "kem", control = con.list)
  saveRDS(POPSIZE2ssm_coho, file=here::here("data", "clean", paste("POPSIZE2ssm_cohoM", scale2, "S4TEST.rds", sep="")))
  coho_time <- proc.time()[3] - ptm
  coho_time
}
# load in ssm_coho
POPSIZE2ssm_coho <- readRDS(file=here::here("data", "clean", paste("POPSIZE2ssm_cohoM", scale2, "S4TEST.rds", sep="")))

# large, medium, and small
if(!file.exists(here::here("data", "clean", paste("POPSIZE3ssm_cohoM", scale3, "S4TEST.rds", sep="")))){
  ptm <- proc.time()
  POPSIZE3ssm_coho <- MARSS(nosa_coho, model = POPSIZE3mod_coho.list, method = "kem", control = con.list)
  saveRDS(POPSIZE3ssm_coho, file=here::here("data", "clean", paste("POPSIZE3ssm_cohoM", scale3, "S4TEST.rds", sep="")))
  coho_time <- proc.time()[3] - ptm
  coho_time
}
# load in ssm_coho
POPSIZE3ssm_coho <- readRDS(file=here::here("data", "clean", paste("POPSIZE3ssm_cohoM", scale3, "S4TEST.rds", sep="")))

# four quarters
if(!file.exists(here::here("data", "clean", paste("POPSIZE4ssm_cohoM", scale4, "S4TEST.rds", sep="")))){
  ptm <- proc.time()
  POPSIZE4ssm_coho <- MARSS(nosa_coho, model = POPSIZE4mod_coho.list, method = "kem", control = con.list)
  saveRDS(POPSIZE4ssm_coho, file=here::here("data", "clean", paste("POPSIZE4ssm_cohoM", scale4, "S4TEST.rds", sep="")))
  coho_time <- proc.time()[3] - ptm
  coho_time
}
# load in ssm_coho
POPSIZE4ssm_coho <- readRDS(file=here::here("data", "clean", paste("POPSIZE4ssm_cohoM", scale4, "S4TEST.rds", sep="")))
...
### STEELHEAD
# dividing into halfs, thirds, and fourths requires twelvths
cutoffs <- quantile(nosa_stel$lnnosa, probs = c(1/12, 1/6, 1/4, 1/3, 5/12, 1/2, 7/12, 2/3, 3/4, 5/6, 11/12), na.rm = TRUE)
                                                                    ...
                                                                    ifelse(nosa_stel$lnnosa <= cutoffs[8], "Q8",
                                                                           ifelse(nosa_stel$lnnosa <= cutoffs[9], "Q9",
                                                                                  ifelse(nosa_stel$lnnosa <= cutoffs[10], "Q10",
                                                                                         ifelse(nosa_stel$lnnosa <= cutoffs[11], "Q11", "Q12"
                                                                                         )))))))))))
...
nosa_stel_rows$method2 <- ifelse(nosa_stel_rows$nth %in% c("Q1", "Q2", "Q3", "Q4", "Q5", "Q6"), "S", "L")
nosa_stel_rows$method2 <- paste0(nosa_stel_rows$methodNo, "",nosa_stel_rows$method2)

nosa_stel_rows$method3 <- ifelse(nosa_stel_rows$nth %in% c("Q1", "Q2", "Q3", "Q4"), "S", 
                                 ifelse(nosa_stel_rows$nth %in% c("Q5", "Q6", "Q7", "Q8"), "M", "L"
                                 ))
nosa_stel_rows$method3 <- paste0(nosa_stel_rows$methodNo, "",nosa_stel_rows$method3)

nosa_stel_rows$method4 <- ifelse(nosa_stel_rows$nth %in% c("Q1", "Q2", "Q3"), "Q1", 
                                 ifelse(nosa_stel_rows$nth %in% c("Q4", "Q5", "Q6"), "Q2",
                                        ifelse(nosa_stel_rows$nth %in% c("Q7", "Q8", "Q9"), "Q3", "Q4"
                                        ))) 
nosa_stel_rows$method4 <- paste0(nosa_stel_rows$methodNo, "",nosa_stel_rows$method4)
...
POPSIZE4R_stel.model <- matrix(list(0), n_stel, n_stel)
diag(POPSIZE4R_stel.model) <- paste0("r", nosa_stel_rows$method4)
...
n_stel <- nrow(nosa_stel)
POPSIZE2R_stel.model <- matrix(list(0), n_stel, n_stel)
diag(POPSIZE2R_stel.model) <- paste0("r", nosa_stel_rows$method2)
...
scale4 <- "9Q2"
  # sets relative value against which other survey methods will be scaled
    # 9 -> dam counts - accurate (according to parsons and Skalski)
    # 9s is the only 9 method that appears across all three species
POPSIZE4a_stel.model <- matrix(list(0), n_stel, 1)
for(i in 1:length(POPSIZE4a_stel.model)){
  if(nosa_stel_rows$method4[i] != scale4){
    POPSIZE4a_stel.model[i] <- paste0("a", nosa_stel_rows$method4[i])
  }
}
...
for(i in 1:length(POPSIZE2a_stel.model)){
  if(nosa_stel_rows$method2[i] != scale2){
    POPSIZE2a_stel.model[i] <- paste0("a", nosa_stel_rows$method2[i])
  }
}
...
POPSIZE2mod_stel.list <- list(
  B = "identity",
  U = "zero",
  Q = "equalvarcov",
  Z = Z_stel.model,
  A = POPSIZE2a_stel.model,
  R = POPSIZE2R_stel.model,
  x0 = "equal",
  V0 = "zero",
  tinitx = 0
)
...
POPSIZE4mod_stel.list <- list(
  B = "identity",
  U = "zero",
  Q = "equalvarcov",
  Z = Z_stel.model,
  A = POPSIZE4a_stel.model,
  R = POPSIZE4R_stel.model,
  x0 = "equal",
  V0 = "zero",
  tinitx = 0
)
...
# agnostic to pop size
if(!file.exists(here::here("data", "clean", paste("POPSIZE1ssm_stelM", scale1, "S4TEST.rds", sep="")))){
  ptm <- proc.time()
  POPSIZE1ssm_stel <- MARSS(nosa_stel, model = POPSIZE1mod_stel.list, method = "kem", control = con.list)
  saveRDS(POPSIZE1ssm_stel, file=here::here("data", "clean", paste("POPSIZE1ssm_stelM", scale1, "S4TEST.rds", sep="")))
  stel_time <- proc.time()[3] - ptm
  stel_time
}
# load in ssm_stel
POPSIZE1ssm_stel <- readRDS(file=here::here("data", "clean", paste("POPSIZE1ssm_stelM", scale1, "S4TEST.rds", sep="")))

# large and small
if(!file.exists(here::here("data", "clean", paste("POPSIZE2ssm_stelM", scale2, "S4TEST.rds", sep="")))){
  ptm <- proc.time()
  POPSIZE2ssm_stel <- MARSS(nosa_stel, model = POPSIZE2mod_stel.list, method = "kem", control = con.list)
  saveRDS(POPSIZE2ssm_stel, file=here::here("data", "clean", paste("POPSIZE2ssm_stelM", scale2, "S4TEST.rds", sep="")))
  stel_time <- proc.time()[3] - ptm
  stel_time
}
# load in ssm_stel
POPSIZE2ssm_stel <- readRDS(file=here::here("data", "clean", paste("POPSIZE2ssm_stelM", scale2, "S4TEST.rds", sep="")))

# large, medium, and small
if(!file.exists(here::here("data", "clean", paste("POPSIZE3ssm_stelM", scale3, "S4TEST.rds", sep="")))){
  ptm <- proc.time()
  POPSIZE3ssm_stel <- MARSS(nosa_stel, model = POPSIZE3mod_stel.list, method = "kem", control = con.list)
  saveRDS(POPSIZE3ssm_stel, file=here::here("data", "clean", paste("POPSIZE3ssm_stelM", scale3, "S4TEST.rds", sep="")))
  stel_time <- proc.time()[3] - ptm
  stel_time
}
# load in ssm_stel
POPSIZE3ssm_stel <- readRDS(file=here::here("data", "clean", paste("POPSIZE3ssm_stelM", scale3, "S4TEST.rds", sep="")))

# four quarters
if(!file.exists(here::here("data", "clean", paste("POPSIZE4ssm_stelM", scale4, "S4TEST.rds", sep="")))){
  ptm <- proc.time()
  POPSIZE4ssm_stel <- MARSS(nosa_stel, model = POPSIZE4mod_stel.list, method = "kem", control = con.list)
  saveRDS(POPSIZE4ssm_stel, file=here::here("data", "clean", paste("POPSIZE4ssm_stelM", scale4, "S4TEST.rds", sep="")))
  stel_time <- proc.time()[3] - ptm
  stel_time
}
# load in ssm_stel
POPSIZE4ssm_stel <- readRDS(file=here::here("data", "clean", paste("POPSIZE4ssm_stelM", scale4, "S4TEST.rds", sep="")))
