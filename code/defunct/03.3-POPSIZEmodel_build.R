...
## repeat the above process but with fifths
# pull in data
load(here("data", "clean", "nosa_codes.Rda"))
nosa <- merge
...
# create scale variable to track whether a population is observed to be "large", "medium", or "small" based on average
cutoffs <- quantile(nosa_chin$lnnosa, probs = c(1/5, 2/5, 3/5, 4/5), na.rm = TRUE)
nosa_chin$scale <- ifelse(nosa_chin$lnnosa <= cutoffs[1], "Q1",
                          ifelse(nosa_chin$lnnosa <= cutoffs[2], "Q2",
                                 ifelse(nosa_chin$lnnosa <= cutoffs[3], "Q3",
                                        ifelse(nosa_chin$lnnosa <= cutoffs[4], "Q4", "Q5"))))
                                        

cutoffs <- quantile(nosa_coho$lnnosa, probs = c(1/5, 2/5, 3/5, 4/5), na.rm = TRUE)
nosa_coho$scale <- ifelse(nosa_coho$lnnosa <= cutoffs[1], "Q1",
                          ifelse(nosa_coho$lnnosa <= cutoffs[2], "Q2",
                                 ifelse(nosa_coho$lnnosa <= cutoffs[3], "Q3",
                                        ifelse(nosa_coho$lnnosa <= cutoffs[4], "Q4", "Q5"))))

cutoffs <- quantile(nosa_stel$lnnosa, probs = c(1/5, 2/5, 3/5, 4/5), na.rm = TRUE)
nosa_stel$scale <- ifelse(nosa_stel$lnnosa <= cutoffs[1], "Q1",
                          ifelse(nosa_stel$lnnosa <= cutoffs[2], "Q2",
                                 ifelse(nosa_stel$lnnosa <= cutoffs[3], "Q3",
                                        ifelse(nosa_stel$lnnosa <= cutoffs[4], "Q4", "Q5"))))
...
# run MARSS model - chin
if(!file.exists(here::here("data", "clean", paste("POPSIZE5ssm_chinM", scale, ".rds", sep="")))){
  ptm <- proc.time()
  POPSIZE5ssm_chin <- MARSS(nosa_chin, model = mod_chin.list, method = "kem", control = con.list)
  saveRDS(POPSIZE5ssm_chin, file=here::here("data", "clean", paste("POPSIZE5ssm_chinM", scale, ".rds", sep="")))
  chin_time <- proc.time()[3] - ptm
  chin_time
}
# load in ssm_chin
POPSIZE5ssm_chin <- readRDS(file=here::here("data", "clean", paste("POPSIZE5ssm_chinM", scale, ".rds", sep="")))


# run MARSS model - coho
if(!file.exists(here::here("data", "clean", paste("POPSIZE5ssm_cohoM", scale, ".rds", sep="")))){
  ptm <- proc.time()
  POPSIZE5ssm_coho <- MARSS(nosa_coho, model = mod_coho.list, method = "kem", control = con.list)
  saveRDS(POPSIZE5ssm_coho, file=here::here("data", "clean", paste("POPSIZE5ssm_cohoM", scale, ".rds", sep="")))
  coho_time <- proc.time()[3] - ptm
  coho_time
}
# load in ssm_coho
POPSIZE5ssm_coho <- readRDS(file=here::here("data", "clean", paste("POPSIZE5ssm_cohoM", scale, ".rds", sep="")))

# run MARSS model - stel
if(!file.exists(here::here("data", "clean", paste("POPSIZE5ssm_stelM", scale, ".rds", sep="")))){
  ptm <- proc.time()
  POPSIZE5ssm_stel <- MARSS(nosa_stel, model = mod_stel.list, method = "kem", control = con.list)
  saveRDS(POPSIZE5ssm_stel, file=here::here("data", "clean", paste("POPSIZE5ssm_stelM", scale, ".rds", sep="")))
  stel_time <- proc.time()[3] - ptm
  stel_time
}
# load in ssm_stel
POPSIZE5ssm_stel <- readRDS(file=here::here("data", "clean", paste("POPSIZE5ssm_stelM", scale, ".rds", sep="")))
