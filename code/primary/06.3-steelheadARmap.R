...
here::i_am("code/primary/06.2-stelARmap.R")
...
sf_facet_data <- sf_stel_nad83col %>%
...
create_facet_plot <- function(data, fill_var, title_text, legend_name) {
  ggplot() +
    ...
    labs(title = title_text,
         ...
         caption = "Data: 1980-2024 Steelhead Surveys") +
    theme_minimal() +
    theme(
...
stel_a_facet <- create_facet_plot(
  sf_facet_data, 
  "mean_a", 
  "Steelhead Survey Bias by DPS", 
  "Bias"
)

# 4. Generate the Precision Facet Plot
stel_r_facet <- create_facet_plot(
  sf_facet_data, 
  "mean_R", 
  "Steelhead Survey Precision by DPS", 
  "Variance"
)
